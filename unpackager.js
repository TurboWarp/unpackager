...
var unpackage = (function(JSZip) {
  'use strict';

  if (!JSZip) {
    if (typeof require === 'function') {
      JSZip = require('@turbowarp/jszip');
    } else {
      throw new Error('JSZip not loaded via script tag and not in Node.js');
    }
  }
  ...
  const readAsText = (data) => {
    if (typeof Blob !== 'undefined' && data instanceof Blob) {
      return new Promise((resolve, reject) => {
        ...
        reader.onerror = () => reject(new Error('Could not read as text'));
    ...
    throw new Error('Could not read as text');
  };
  ...
  const unpackage = async (data) => {
    const packagedZip = await unzipOrNull(data);
    ...
    const dataURIMatch = (
      ...
      // HTML files generated by some versions of HTMLifier use an inline base64 URL called in a variable called "FILE"
      text.match(/^FILE = "(data:application\/octet-stream;base64,[a-zA-Z0-9+/=]+)",$/m)
    );
    if (dataURIMatch) {
...
// Allow usage as Node.js dependency
